...
class DemonBotPlugin(Star):
    ...
    def _migrate_config(self):
        ...
        """
        try:
            version = self.config.get("_schema_version") or 0
            if version >= 5:
                return
            if version >= 4:
    ...
    async def on_llm_request(self, event: AstrMessageEvent, req):
        ...
        if self._cfg("members", "inject_recent_context", default=True):
            ctx = self._recent_context(group_id, self._cfg("members", "recent_context_lines", default=4))
            if ctx:
    ...
    def _trim_request_context(self, req) -> None:
        ...
        if not self._cfg("token_saver", "enabled", default=True):
            return
        max_msgs = self._cfg("token_saver", "max_context_messages", default=6)
        max_chars = self._cfg("token_saver", "max_context_chars_per_message", default=120)
        try:
            ...
            cap = self._cfg("token_saver", "max_system_prompt_chars", default=1200)
